...
const isExerciseOwnerOrAdmin = async (req, res, next) => {
    try {
        ...
        if (req.user?.is_admin || exercise?.user_id === req.user?.id) {
            return next();
        }
...
const isExerciseGlobal = async (req, res, next) => {
    try {
        ...
        if(exercise.is_global && !req.user.is_admin) {
            return res.status(403).json({ error: 'Global exercise cannot be edited or deleted' })
        }
